...
export const Route = createFileRoute("/_authenticated/products/$modelId")({
  head: () => ({
    meta: [{ title: "Product — Datasentinel" }, { name: "robots", content: "noindex" }],
  }),
...
async function fetchProduct(modelId: string) {
  const [model, agg, risk, cluster, history, alerts, invs] = await Promise.all([
    supabase
      .from("product_models")
      .select(
        "id, external_product_id, name, sku, state_code, warranty_default, family_id, product_families(name)",
      )
      .eq("id", modelId)
      .maybeSingle(),
    ...
    return (
      <div className="p-6">
        <EmptyState
          ...
          description="Este produto pode ter sido removido."
        />
      </div>
    );
  }
  ...
  return (
    <div>
      <PageHeader
        ...
        description={
          <span>
            Família: {(m as any).product_families?.name ?? "—"} · Estado: {m.state_code ?? "—"} ·
            Garantia: {m.warranty_default ?? "—"}
          </span>
        }
        actions={
          <div className="flex items-center gap-2">
            <Button variant="ghost" size="sm" asChild>
              <Link to="/ranking">
                <ArrowLeft className="mr-2 h-4 w-4" /> Voltar para o ranking
              </Link>
            </Button>
      ...
      <div className="space-y-6 px-6 py-6">
              ...
              {r ? (
                <div className="mt-2 space-y-2">
                  ...
                  <p className="font-mono text-xs text-muted-foreground">
                    confiança {(Number(r.confidence) * 100).toFixed(0)}% · v{r.model_version}
                  </p>
                </div>
              ) : (
                <p className="mt-2 text-sm text-muted-foreground">Nenhuma previsão ainda</p>
              )}
            </CardContent>
          </Card>
          <Kpi label="Total de OS" value={fmtNum(a?.total_os)} />
          <Kpi
            label="% de OS Alta"
            value={a ? `${Number(a.high_os_percentage).toFixed(1)}%` : "—"}
          />
          <Kpi
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Probabilidades da classe de risco</CardTitle>
            </CardHeader>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Cluster de modo de falha</CardTitle>
            </CardHeader>
            <CardContent>
              {data.cluster ? (
                <div className="space-y-2">
                  ...
                  <p className="font-mono text-xs text-muted-foreground">
                    modelo v{data.cluster.model_version} · {fmtDate(data.cluster.inferred_at)}
                  </p>
                ...
                <p className="text-sm text-muted-foreground">Nenhum cluster atribuído</p>
              )}
            </CardContent>
          </Card>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Histórico de risco</CardTitle>
            </CardHeader>
            <CardContent>
              {!data.history.length ? (
                <p className="text-sm text-muted-foreground">Nenhuma transição ainda.</p>
              ) : (
                    ...
                    return (
                      <li key={h.id} className="flex items-center justify-between py-2 text-sm">
                        <span className="font-mono text-xs">
                          {CLASS_LABELS[h.previous_class] ?? h.previous_class ?? "—"} →{" "}
                          <b>{CLASS_LABELS[h.new_class] ?? h.new_class}</b> ·{" "}
                          {Number(h.new_score).toFixed(3)}
                        </span>
                  ...
                  {data.alerts.map((al: any) => (
                    ...
                    >
                      ...
                      <StatusBadge
                        ...
                        tone={al.status === "open" ? "warning" : "success"}
                      />
                    </li>
                  ))}
                </ul>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Investigações</CardTitle>
          </CardHeader>
          <CardContent>
            {!data.investigations.length ? (
              <p className="text-sm text-muted-foreground">Nenhuma investigação aberta.</p>
            ) : (
              <ul className="divide-y divide-border">
                  ...
                  return (
                    <li key={i.id} className="flex items-center justify-between py-2 text-sm">
                      ...
                      <div className="flex items-center gap-3">
                        <StatusBadge
                          label={INV_STATUS_LABELS[norm] ?? norm}
...
function OpenInvestigationDialog({ modelId, alertId }: { modelId: string; alertId?: string }) {
  ...
  const mut = useMutation({
    ...
    onError: (e: any) => toast.error(e?.message ?? "Falha ao abrir investigação"),
  });
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button size="sm">
          <FilePlus2 className="mr-2 h-4 w-4" /> Abrir investigação
        </Button>
      </DialogTrigger>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Abrir investigação</DialogTitle>
        </DialogHeader>
